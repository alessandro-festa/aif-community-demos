...
Airflow ─ generate_dataset → Postgres (customers/families/policies/claims/tickets)
        ├ index_cases      → embed tickets → Qdrant support_cases (semantic search)
        └ customize_model  → Ollama persona model `support-agent`

Local chat UI ─ chat + tools → Ollama qwen2.5:0.5b (open/close ticket)
...
| Qdrant | `qdrant` (suse-ai-registry) | Lightweight vector DB; semantic case index at `qdrant:6333` (REST). |
| Ollama | `ollama` (application-collection) | CPU; `qwen2.5:0.5b` (chat + tool-calling), `qwen2.5vl:3b` (accident-photo vision), `nomic-embed-text`. |
...
Airflow's own Postgres metadata backend), and talk to the embedding endpoint + Qdrant
over HTTP.
