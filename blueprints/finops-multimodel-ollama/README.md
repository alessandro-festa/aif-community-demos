# FinOps Multi-Model Gateway (Ollama, CPU)

A **FinOps** demo for SUSE AI Factory built on top of the `litellm-guardrails`
gateway. It turns a guarded LLM proxy into a **multi-model, cost-instrumented**
workload and reconciles the two cost planes every AI platform has:
...
OpenCost (infra $) ──────────────────────────────────────►/metrics ┤►  Prometheus ─► Grafana
...
- **Infra cost** — **OpenCost** allocates the $ of the pods (CPU/RAM/PV) into Prometheus.
...
|-----------|--------------|-------|
| Ollama | `ollama` (`application-collection`) | CPU; pulls 3 models of different sizes/prices |
...
| OpenCost | `opencost` (`application-collection`) | infra $ → Prometheus (queried via the in-namespace Prometheus) |
...
1. `finops_setup` — creates a **virtual key + monthly budget** per team
   (engineering, data-science, marketing, support). Idempotent; uses fixed key values
   (`sk-team-*`). Requires `store_model_in_db: true`.
...
- a default StorageClass (Ollama, Postgres, Prometheus, Grafana use PVCs);
- cert-manager.

## Run the demo

1. In the marketplace, open **FinOps Multi-Model Gateway (Ollama)**, choose guardrails,
   and **Import**.
2. In the SUSE AI Factory UI: **Create AIWorkload**, pick a namespace, deploy. Wait for
   all components to be Ready (large workload — several minutes).
...
4. Open **Grafana** → *FinOps — Multi-Model Gateway*. Explore token spend, chargeback,
   history and infra cost. Use **OpenCost UI** for per-workload infra detail.
...
- **Demo only, unsupported.** Master key (`sk-guardrails-demo`), team keys
  (`sk-team-*`), Postgres/Grafana passwords are demo placeholders — change them for
  anything real.
- **Bundled Postgres service name.** The exporter points at `litellm-postgresql`
  (the litellm chart's standalone Postgres). If your release names it differently,
  update `LITELLM_DB_HOST` in the exporter (`kubectl -n <ns> get svc`).
...
- **OpenCost is configured against the cluster out of the box.** It queries this
  ...
  cluster-wide **cadvisor** (via the kubelet) + **kube-state-metrics**. OpenCost emits
  node-cost metrics itself using its **default on-prem pricing** (so costs are
  realistic but synthetic — no cloud billing integration). Allocation data appears a
  few minutes after the pods are Ready and Prometheus has scraped a couple of cycles.
...
- **No custom Airflow image.** Uses the stock `apache-airflow` image — the DAGs need
  only `requests` (already present); the Prometheus remote-write client in
  `dags/common.py` is pure-stdlib (hand-rolled protobuf + snappy).
- **Chart versions** are pinned to the Application Collection versions available at
  authoring time (prometheus 29.13.1, grafana 12.7.2, opencost 2.5.27). Re-pin with
  `helm show chart oci://dp.apps.rancher.io/charts/<name>` if needed.
