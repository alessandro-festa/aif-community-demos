...
# --------------------------------------------------------------------------- #
# Deterministic hashing embedding — MUST match dags/dora_common.hash_embed exactly.
# --------------------------------------------------------------------------- #
...
def _qdrant_post(path: str, body: dict) -> dict:
    ...
    r = requests.post(f"{QDRANT_URL}{path}", json=body, headers=headers, timeout=HTTP_TIMEOUT)
    r.raise_for_status()
    data = r.json()
...
def qdrant_ok() -> bool:
    try:
        ...
        r = requests.get(f"{QDRANT_URL}/collections/{INCIDENTS_COLLECTION}",
...
def qdrant_search_incidents(query: str, top_k: int = 5) -> list[dict]:
    data = _qdrant_post(f"/collections/{INCIDENTS_COLLECTION}/points/search", {
        "vector": hash_embed(query),
        "limit": top_k,
        "with_payload": True,
    })
    hits = []
    for h in (data.get("result") or []):
        hits.append({**(h.get("payload") or {}), "distance": h.get("score")})
    return hits
